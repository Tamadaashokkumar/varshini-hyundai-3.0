...
import Link from "next/link";
import { motion, AnimatePresence } from "framer-motion";
import Image from "next/image"; // 🔥 PERF UPDATE: Next.js Image
    ...
    <section className="relative w-full h-[100dvh] overflow-hidden font-sans bg-[#030303]">
      <AnimatePresence initial={false} mode="sync">
        <motion.div
          ...
          animate="center"
          exit="exit"
          className="absolute inset-0 w-full h-full flex items-center bg-[#0a0a0a]" // 🔥 PERF UPDATE: Placeholder background color
        >
          {/* ================= BACKGROUND LAYER ================= */}
          <div className="absolute inset-0 w-full h-full overflow-hidden">
            {currentSlide.image && (
              <motion.div
                ...
                className="absolute inset-0 w-full h-full"
              >
                {/* 🔥 PERF UPDATE: Replaced img with next/image */}
                <Image
                  src={currentSlide.image}
                  alt={currentSlide.title}
                  fill
                  ...
                  // మొదటి స్లైడ్ కి మాత్రమే priority=true ఇస్తాము. మిగతావి లేజీ లోడ్ అవుతాయి.
                  priority={current === 0}
                  quality={85} // క్వాలిటీ 85 కి సెట్ చేసాను (స్పీడ్ పెరుగుతుంది)
                />
              </motion.div>
            )}
