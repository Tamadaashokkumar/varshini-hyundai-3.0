...
apiClient.interceptors.request.use(
  (config: InternalAxiosRequestConfig) => {
    // ఇక్కడ భవిష్యత్తులో CSRF టోకెన్ లాంటివి కలపడానికి వాడుకోవచ్చు
    return config;
  },
  (error: AxiosError) => {
    return Promise.reject(error);
...
apiClient.interceptors.response.use(
  ...
  async (error: AxiosError) => {
    ...
    if (error.response.status === 401 && !originalRequest._retry) {
      ...
      if (isRefreshing) {
        return new Promise((resolve, reject) => {
          ...
          .then(() => {
            // 🔥 FIX 1: క్యూ లోంచి మళ్ళీ వెళ్ళేటప్పుడు కూడా _retry ట్రూ చేయాలి. లేదంటే లూప్ అవుతుంది
            originalRequest._retry = true;
            return apiClient(originalRequest);
          })
          .catch((err) => {
      ...
      try {
        // నేరుగా axios వాడి రిఫ్రెష్ చేయడం (apiClient వాడితే మళ్ళీ ఇక్కడికే వస్తుంది కాబట్టి)
        await axios.post(
          `${API_BASE_URL}/auth/refresh-token`,
          {},
          { withCredentials: true },
        ...
        console.error("🔥 [Interceptor]: Refresh Token API Failed!");

        // క్యూలో ఉన్నవన్నీ రిజెక్ట్ చేయి
        processQueue(refreshError);

        // 🔥 FIX 2: Refresh ఫెయిల్ అయిందంటే యూజర్ పూర్తిగా లాగౌట్ అవ్వాలి
        // (Next.js క్లయింట్ సైడ్ అయితేనే ఇది రన్ అవుతుంది)
        if (typeof window !== "undefined") {
          // మీరు కావాలంటే localStorage.clear() లాంటివి ఇక్కడ చేయొచ్చు
    ...
    // 401 కాకుండా వేరే ఎర్రర్ (500, 404) వస్తే డైరెక్ట్ గా రిజెక్ట్
    return Promise.reject(error);
  },
);
